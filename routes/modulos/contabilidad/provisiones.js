...
router.post('/reporte_programacion_vencimientos', async (req, res) => {
    try {
        ...
        .query(
            ...
            "datediff(day,fecha_emision, case fecha_vencimiento when  \n"+
            "'1900-01-01 00:00:00.000' then fecha_emision else fecha_vencimiento end ) as dias_vencidos,  \n"+
            "moneda_doc as moneda,  \n"+
            "tc,  \n"+
            ...
            "round(  \n"+
            "(Case cmoneda_docref When 'S/' then  \n"+
            "(CASE hmovcond.d_h when 'D' then round(hmovcond.mndebe,2) ELSE (Case When Left(LTRim(RTrim(hmovcond.ccuenta)),3) In ('122') THEN  \n"+
            "round(-hmovcond.mnhaber,2) ELSE round(hmovcond.mnhaber,2) END) END) * Htipdoc.valor_signo  \n"+
            "Else \n"+
            "(Case When @tasa_cambio > 0 then (  \n"+
            "(CASE hmovcond.d_h when 'D' then round(hmovcond.medebe,2) ELSE (Case When Left(LTRim(RTrim(hmovcond.ccuenta)),3) In ('122') THEN  \n"+
            "round(-hmovcond.mEhaber,2) ELSE round(hmovcond.mEhaber,2) END) END) * Htipdoc.valor_signo) * @tasa_cambio  \n"+
            "Else (CASE hmovcond.d_h when 'D' then round(hmovcond.mndebe,2)  \n"+
            "ELSE  \n"+
            "round(hmovcond.mnhaber,2) END) * Htipdoc.valor_signo End)  \n"+
            "End),2)AS importe_soles,  \n"+
            "Round((Case Hmovcond.cmoneda_docref when 'S/'  \n"+
            "then  \n"+
            "(CASE When Left(LTRim(RTrim(hmovcond.ccuenta)),3) In ('122') THEN  \n"+
            "[dbo].[Udf_Monto_Pago_Documentos3](@codigo_empresa,@tipo_anexo,Hanexos.ccod_anexo,hmovcond.ccod_svoucher, \n"+
            "hmovcond.ccuenta,Hmovcond.ctip_docref,hmovcond.cnum_serieref,hmovcond.cnum_docref,'S/',@fecha_final) * Htipdoc.valor_signo  \n"+
            "ELSE  \n"+
            "[dbo].[Udf_Monto_Pago_Documentos3](@codigo_empresa,@tipo_anexo,Hanexos.ccod_anexo,hmovcond.ccod_svoucher, \n"+
            "hmovcond.ccuenta,Hmovcond.ctip_docref,hmovcond.cnum_serieref,hmovcond.cnum_docref,'S/',@fecha_final)  \n"+
            "* Htipdoc.valor_signo  \n"+
            "END)  \n"+
            "Else  \n"+
            "(Case when hmovconc.tipo_cambio> 0 then  \n"+
            "[dbo].[Udf_Monto_Pago_Documentos3](@codigo_empresa,@tipo_anexo,Hanexos.ccod_anexo,hmovcond.ccod_svoucher, \n"+
            "hmovcond.ccuenta,Hmovcond.ctip_docref,hmovcond.cnum_serieref,hmovcond.cnum_docref,'S/',@fecha_final)  \n"+
            "* Htipdoc.valor_signo End)  \n"+
            "End),2) AS pago_soles,  \n"+
            "round((  \n"+
            "Case cmoneda_docref When '$' then  \n"+
            "(CASE hmovcond.d_h when 'D' then round(hmovcond.medebe,2) ELSE  \n"+
            "(Case When Left(LTRim(RTrim(hmovcond.ccuenta)),3) In ('122') THEN  \n"+
            "round(-hmovcond.mEhaber,2) ELSE round(hmovcond.mEhaber,2) END) END) * Htipdoc.valor_signo  \n"+
            "Else  \n"+
            "(Case When @tasa_cambio > 0 then (  \n"+
            "(CASE hmovcond.d_h when 'D' then round(hmovcond.mndebe,2) ELSE  \n"+
            "(Case When Left(LTRim(RTrim(hmovcond.ccuenta)),3) In ('122') THEN  \n"+
            "round(-hmovcond.mnhaber,2) ELSE round(hmovcond.mnhaber,2) END) END) * Htipdoc.valor_signo) / @tasa_cambio Else (CASE  \n"+
            "hmovcond.d_h when 'D' then round(hmovcond.medebe,2) ELSE  \n"+
            "round(hmovcond.mehaber,2) END) * Htipdoc.valor_signo End)  \n"+
            "End  \n"+
            ")  \n"+
            ",2)AS importe_dolares,  \n"+
            "(Case Hmovcond.cmoneda_docref when '$'  \n"+
            "then  \n"+
            "(CASE When Left(LTRim(RTrim(hmovcond.ccuenta)),3) In ('122') THEN  \n"+
            "[dbo].[Udf_Monto_Pago_Documentos3](@codigo_empresa,@tipo_anexo,Hanexos.ccod_anexo,hmovcond.ccod_svoucher, \n"+
            "hmovcond.ccuenta,Hmovcond.ctip_docref,hmovcond.cnum_serieref,hmovcond.cnum_docref,'$',@fecha_final)  \n"+
            "* Htipdoc.valor_signo  \n"+
            "ELSE  \n"+
            "[dbo].[Udf_Monto_Pago_Documentos4](@codigo_empresa,@tipo_anexo,Hanexos.ccod_anexo,hmovcond.ccod_svoucher, \n"+
            "hmovcond.ccuenta,Hmovcond.ctip_docref,hmovcond.cnum_serieref,hmovcond.cnum_docref,'$',@fecha_final)  \n"+
            "* Htipdoc.valor_signo  \n"+
            "END)  \n"+
            "else \n"+
            "(Case when hmovconc.tipo_cambio > 0 then  \n"+
            "[dbo].[Udf_Monto_Pago_Documentos](@codigo_empresa,@tipo_anexo,Hanexos.ccod_anexo,hmovcond.ccod_svoucher, \n"+
            "hmovcond.ccuenta,Hmovcond.ctip_docref,hmovcond.cnum_serieref,hmovcond.cnum_docref,'$',@fecha_final)  \n"+
            "* Htipdoc.valor_signo end)  \n"+
            "End) AS pago_dolares,  \n"+
            ...
            "isnull(erp_motivos_tramite.erp_nommot,'Ninguno')Motivo_Nombre, upper(hciudad.cnom_ciudad) as Ciudad,  \n"+
...
router.get('/programacion_vencimientos/:tipo',isLoggedin,async (req, res) => {
    ...
    // res.render("modulos/contabilidad/provisiones",{cob_pag: cob_pag, subvoucher: subvoucher});
    res.render("modulos/contabilidad/programacion_vencimientos",{tipo: tipo});
});
