...
router.post('/lista', async (req, res) => {
    try {
        ...
        " from erp_concepto1 where erp_codemp = @Codigo_Empresa order by erp_codcon asc";
...
router.post('/eliminar', async (req, res) => {
  try {
    var query ="delete from erp_concepto1 where erp_codemp = @Codigo_Empresa and erp_codcon = @erp_codcon \n"
    ...
    await transaction.begin(async err => {
        try{
          ...
          .query(            
           ...
           " where Harticul.ccod_empresa = @ccod_empresa and Harticul.codmarca = @codigo \n"
          );
