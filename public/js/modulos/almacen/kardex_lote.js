...
function cargar_lista_productos(){
    ...
    grid_lista_productos.jqGrid({
        ...
        colModel: [
            ...
            {name: 'Lote', template: textTemplate, align: 'center'},
            {name: 'Vencimiento', template: textTemplate, align: 'center'},
...
function cargar_kardex(){
    ...
    for(var i=0; i<ids.length;i++){
        ...
        lotes.push(row.Lote);
        vencimientos.push(row.Vencimiento);
    }
    $.ajax({
        type: 'POST',
...
function rellenarKardex(data){
    ...
    var agrupacion_lote="";
    var agrupacion_vencimiento="";
    var nuevo_grupo=false;
    for(var i = 0; i<data.length; i++){
        var detalle = data[i];
        if(agrupacion_codigo!=detalle.Codigo || agrupacion_lote != detalle.nro_lote || agrupacion_vencimiento != detalle.vcto){
            Cantidad_Saldo=0;
            ...
            agrupacion_lote=detalle.nro_lote;
            agrupacion_vencimiento=detalle.vcto;
            nuevo_grupo=true;
        }else{
            nuevo_grupo=false;
        }
        if((header_height + footer_height + content_height < page_height ) && (page_number!=0)){
            ...
            new_fila = new_fila.replace("print_body_nro_lote",detalle.nro_lote)
            ...
            new_page = new_page.replace("print_body_nro_lote", detalle.nro_lote)
