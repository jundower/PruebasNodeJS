    ...
    if (detraccion_check == "S" && jq_monto_detraccion.val() > 0) {
        await $.ajax({
            ...
            success: function (data) {
                ...
                FilaDetra.Cuenta = jq_moneda.val() == 'S/' ? row.Cuenta_Detraccion_Soles: row.Cuenta_Detraccion_Dolares;
                FilaDetra.Nombre_Cuenta = jq_moneda.val() == 'S/' ? row.NombreCuenta_Detraccion_Soles: row.NombreCuenta_Detraccion_Dolares;
                FilaDetra.Tipo_Origen = 'I';
                FilaDetra.Tipo_Referencia  ='DT';
...
function guardar_2() {
    ...
    rows.forEach(rowid => {
        var rowdata = jq_grid_detalle_contable.getRowData(rowid);
        console.log(rowdata.Tipo_Anexo);
        rowdata.CenCos=$("#lista_select_cencos_"+rowid).val();
        rowdata.Ot=$("#lista_select_ot_"+rowid).val();
        rowdata.Unidad_Negocio=$("#lista_select_unidad_negocio_"+rowid).val();
        rowdata.Tipo_Referencia=$("#lista_select_tipo_documento_"+rowid).val();
...
async function agregar_cabecera_pendientes(rowData){
    ...
    var Codigo_Centro_Costos = rowData.Cencos;
    var Codigo_Orden_Trabajo = rowData.Ot;
    var Codigo_Unidad_Negocio = rowData.Codigo_Unidad_Negocio;
