...
var url_configuracion_permisos = '/configuraciones/configuracion_permisos/proceso_financiero__sist_caja_bancos__cancelacion_cobros';
...
$(document).ready(function() {
    ...
    jq_codigo_caja_banco.change(function(){
        ...
        rellenar_nombre_banco($(this).val());
    });
...
async function nuevo(){
    ...
    cargar_periodo_ejercicio_contable('03',true);
    rellenar_codigo_nombre(url_subvoucher_lista,"subvoucher",'','03');
    rellenar_codigo_nombre(url_tipo_auxiliar_lista,"tipo_anexo",'','12');
    rellenar_codigo_nombre(url_tipo_documento,"tipo_referencia1");
...
function guardar(){
    ...
    rows.forEach(rowid => {
        ...
        // getDateFormat(jq_fecha_referencia2.val(),'y-m-d','d/m/y','-','/')
        // Fecha_Vencimiento: "04/02/2021"  
...
function validar(){
    ...
    if(jq_tipo_referencia2.val()=="00" && isEmptyOrWhiteSpaces(jq_serie_referencia2.val()) && isEmptyOrWhiteSpaces(jq_numero_referencia2.val())){
        ...
        jq_fecha_referencia2.val(getDateFormat(fila1.Fecha_Referencia,'d/m/y','y-m-d','/','-'));
        retorno = false;
...
async function consultar(impresion) {
    ...
    if(impresion){
        var ejercicio_actual = new Date();  
        data ={
            cob_pag: "",
            ejercicio: ejercicio_actual.getFullYear(),
            periodo: getPeriodo(jq_fecha_registro.val()),
...
function documentos_pendientes_contable_finalizar(){
    jq_ingresos_egresos.val()=="ING" ? jq_tipo_anexo.val("12") : jq_tipo_anexo.val("42");
    jq_ingresos_egresos.val()=="ING" ? jq_subvoucher.val("03") : jq_subvoucher.val("04");
    jq_subvoucher.trigger("change");
    jq_tipo_anexo.trigger("change");
    var jq_grid=jq_grid_detalle_contable;
...
function rellenar_nombre_banco(codigo) {
    $.ajax({
        type:'POST',
        url: '/cuentas_corrientes/lista/'+codigo,
        success: function (data) {
            console.log(data[0].Descripcion_Banco)
            jq_nombre_banco.val(data[0].Descripcion_Banco);
        }
    })
}