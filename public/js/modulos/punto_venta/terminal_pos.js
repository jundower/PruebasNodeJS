...
var url_guardar = "/terminal_pos/guardar";
var url_configuracion_permisos = '/configuraciones/configuracion_permisos/proceso_pto_venta__sist_pto_venta__terminal_pos';
...
var caja = "caja";
var monto_soles = "monto_soles";
var monto_dolares = "monto_dolares";
var total_cobrar = "total_cobrar";
var pago = "pago";
...
var jq_caja;
var jq_monto_soles;
var jq_monto_dolares;
var jq_total_cobrar;
var jq_pago;
var jq_total_cobrar_dolares;
var jq_tc_cancelacion;
var jq_vuelto_soles;
var jq_vuelto_dolares;
var jq_pago_restante_soles;
var jq_pago_restante_dolares;
var datos_anticipos=[];
var cancelacion = "N";
var codigo_movimiento = "EF-S";
var nombre_movimiento = "EFECTIVO_SOLES";
var concepto_movimiento = "EFECTIVO";
...
$(document).ready(function() {
    ...
    jq_cobrador = $("#"+cobrador);
    jq_caja = $("#"+caja);
    jq_monto_soles = $("#"+monto_soles);
    jq_monto_dolares = $("#"+monto_dolares);
    jq_total_cobrar = $("#"+total_cobrar);
    jq_pago = $("#"+pago);
    jq_total_cobrar_dolares = $("#total_cobrar_dolares");
    jq_tc_cancelacion = $("#tc_cancelacion");
    jq_vuelto_soles = $("#vuelto_soles");
    jq_vuelto_dolares = $("#vuelto_dolares");
    jq_pago_restante_soles = $("#pago_restante_soles");
    jq_pago_restante_dolares = $("#pago_restante_dolares");
    ...
    jq_tipo.change(function() {
        ...
        switch ($(this).val()){
            case "FACTURAR GUIAS DE VENTA":
                ...
                setVentasPendientes('Documentos Pendientes','/terminal_pos/lista_documentos_pendientes','/terminal_pos/lista_detalle_pendientes','09');
                break;
            // case "ANTICIPO":
            case "NOTA DE DEBITO":
                $("#buscar_pendientes").prop( "disabled", false );
                setVentasPendientes('Documentos Pendientes','/terminal_pos/lista_documentos_pendientes','/terminal_pos/lista_detalle_pendientes','08');
            ...
            case "NOTA DE CREDITO POR DESCUENTO":
                $("#buscar_pendientes").prop( "disabled", false );
                setVentasPendientes('Documentos Pendientes','/terminal_pos/lista_documentos_pendientes','/terminal_pos/lista_detalle_pendientes','07');
                jq_tipo_documento.val("07");
                jq_motivo_operacion_div.show();
                Llenar_Motivo(jq_tipo.val());
                $("#modal_pendientes").modal("show");
            ...
            case "FACTURAR PEDIDO PARCIAL":
                $("#buscar_pendientes").prop( "disabled", false );
                setVentasPendientes('Documentos Pendientes','/toma_pedidos/lista_documentos_pendientes','/toma_pedidos/lista_detalle_pendientes');
                $("#modal_pendientes").modal("show");
                break;
            default:
                $("#buscar_pendientes").prop( "disabled", true );
    ...
    jq_monto_soles.keyup(function () {
        jq_monto_dolares.val(0.00);
        calcular_cancelacion($(this).val(), jq_tasa_cambio.val(), jq_moneda.val(),'Soles');
    });
    jq_monto_dolares.keyup(function () {
        jq_monto_soles.val(0.00);
        calcular_cancelacion($(this).val(), jq_tasa_cambio.val(), jq_moneda.val(),'Dolares');
    });
    ...
    $("#imprimir").show();
    $('#modificar').hide();
    setTitle("Terminal POS");
    setTitleLista("Listado de Facturas");
    setListaDocumentos('/terminal_pos/lista', '/terminal_pos/consultar', '/terminal_pos/detalle','Motivo','listado_documentos');
...
function guardarDocumento(){
    ...
    switch (estado) {
        case 'guardar':
            url_guardar = "/terminal_pos/guardar";
            mensaje = "Guardado satisfactoriamente."
            impresion=true;
            break;
        case  'modificar':
            url_guardar = "/terminal_pos/modificar";
            mensaje = "Modificado satisfactoriamente."
            break;
        case 'eliminar':
            url_guardar = "/terminal_pos/eliminar";
            mensaje = "Eliminado satisfactoriamente."
            break;
        case 'anular':
            url_guardar = "/terminal_pos/anular";
    ...
    $.ajax({
        ...
        data:{
            ...
            erp_codcaja: jq_caja.val(),
            erp_vuelto: jq_moneda.val() == "S/"? (jq_vuelto_soles.text() * 1): (jq_vuelto_dolares.text() * 1),
...
function cargar_cobrador() {
    $.ajax({
        type: 'GET',
        url: '/configuracion_cobrador/cobrador',
        success: function (lists) {
            var data = lists[0]
            if (data != undefined) {
                jq_cobrador.val(data.Codigo +' - '+data.Usuario)
                lists.forEach(list=>{
                    jq_caja.append('<option value="'+list.Cta_Cteb+'" >'+data.Cta_Cteb+' - '+data.Nombre_Cta_Cteb+ '</option>');
                });
            }
        }
    })
...
$(window).keydown(function(event) {
    ...
    switch (keycode){
        case 120:
            if (isEmptyOrWhiteSpaces(jq_codigo_cliente.val())) {
                mostrarMensajeModal("Aviso","Debe elegir un cliente");
                return true;
            }
            if(validarDetalleVentas()) {
                mostrarMensajeModal("Aviso","Debe ingresar un detalle de documento");
                ...
                var t = Number(jq_total.val())
                var tc = Number(jq_tasa_cambio.val())
                if (jq_moneda.val() == 'S/') {
                    jq_monto_soles.val(t);
                    jq_total_cobrar.text(t);
                    jq_total_cobrar_dolares.text((t/tc).toFixed(2));
                }else{
                    
                    jq_monto_soles.val((t*tc).toFixed(2));
                    jq_total_cobrar.text((t*tc).toFixed(2));
                    jq_total_cobrar_dolares.text(t);
                }
                
                calcular_cancelacion(jq_total.val(), jq_tasa_cambio.val(), jq_moneda.val(), '');
...
$("#ok_cancelacion").click(function () {
    var total_s = Number(jq_total_cobrar.text());
    var total_d = Number(jq_total_cobrar_dolares.text());
    var pago_s_d = Number(jq_pago.text());
    if (jq_moneda.val() == "S/"? total_s <= pago_s_d: total_d <= pago_s_d) {
        cancelacion = "S";
        $("#modal_cancelacion").modal("hide");
        guardar();
    }else{
        mostrarMensajeModal("Aviso","El Monto Recibido Es Menor Al Monto A Pagar");
    }
})

function calcular_cancelacion(total, tc, moneda, input) {
    jq_pago.text(total);
    jq_tc_cancelacion.text(tc)
    if (moneda == "S/" ) {
        if (total > Number(jq_total_cobrar.text())) {

            var vuelto_s = total - Number(jq_total_cobrar.text());
            var vuelto_d = vuelto_s / tc;

            jq_vuelto_soles.text((vuelto_s).toFixed(2));
            jq_vuelto_dolares.text((vuelto_d).toFixed(2));
            jq_pago_restante_soles.text("0.00");
            jq_pago_restante_dolares.text("0.00");
        }
        else if (total < Number(jq_total_cobrar.text())) {
            var p_restante_s = Number(jq_total_cobrar.text()) - total;
            var p_restante_d = p_restante_s / tc;

            jq_pago_restante_soles.text((p_restante_s).toFixed(2));
            jq_pago_restante_dolares.text((p_restante_d).toFixed(2));
            jq_vuelto_soles.text("0.00");
            jq_vuelto_dolares.text("0.00");
        }else{
            jq_vuelto_soles.text("0.00");
            jq_vuelto_dolares.text("0.00");
            jq_pago_restante_soles.text("0.00");
        ...
        console.log(total);
        if (total > Number(jq_total_cobrar_dolares.text())) {

            
            var vuelto_d = total - Number(jq_total_cobrar_dolares.text());
            var vuelto_s = vuelto_d * tc; 
        ...
        else if (total < Number(jq_total_cobrar_dolares.text())) {
            console.log(total);
            console.log(jq_total_cobrar_dolares.text());
            var p_restante_d = Number(jq_total_cobrar_dolares.text()) - total;
            var p_restante_s = p_restante_d * tc; 
