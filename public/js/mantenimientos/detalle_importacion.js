...
Input Text:
    ...
    - igv
    - total
calcular_totales_importacion se usa 3 veces, al ingresar nuevos productos, al modificar una linea o al cambiar la opción del "mas igv"
*/
var url_detalle_venta = "/dua/detalle";
var url_configuracion_grid="/configurar_tablas/generar";

var grid_detalle_importacion = "grid_detalle_importacion"
var mas_igv = "mas_igv";
...
var jq_grid_detalle_importacion;
...
var lista_ot="";
var lista_signo = "";
var row_editing_detalle_importacion;
$(document).ready(function() {
    jq_grid_detalle_importacion = $("#"+grid_detalle_importacion);
    jq_mas_igv = $("#"+mas_igv);
...
$(window).bind('resize', function() {
    resize_jqgrid_restar(jq_grid_detalle_importacion,$("#myTabContent").height() + $("#myTab").height() + $("#nav_title_bar").height() + 100,$(window).width() - $("#myTabContent").width() );
});
...
$(window).keydown(function(event) {
    ...
    if(id_focused!=undefined){
        switch(id_focused){
            case "grid_detalle_importacion_kn":
                keydown_detalle_ventas(keycode);
                break;
            case "grid_detalle_importacion":
                keydown_detalle_ventas(keycode);
                break;
        }
    }else{
...
function cargar_configuracion_detalle_ventas(){
    $.ajax({
        ...
        success: function (lists){
            cargar_jq_grid_detalle_importacion(lists);
        }
    });
}

async function cargar_jq_grid_detalle_importacion(configuraciones){
    ...
    var jq_grid=jq_grid_detalle_importacion;

    var array = [{"Nombre":"+","Codigo": "+",},{"Nombre":"-","Codigo": "-",}]
    for (let index = 0; index < array.length; index++) {
        const element = array[index];
        lista_signo +=  '<option value ="'+element.Codigo+'">'+element.Nombre+'</option>'
    }
    ...
    for(var i=3 ; i<Object.keys(configuracion_visibilidad).length; i++)
    {
        ...
        }else if(col_nombre == "Signo"){
            col_Model_fila.formatter = lista_select_signo;

            col_model_grid_detalle_productos.push(col_Model_fila);
        }else if(col_nombre == "Cencos"){
            col_Model_fila.formatter = lista_select_cencos;

            col_model_grid_detalle_productos.push(col_Model_fila);
        }else if(col_nombre == "Ot"){
    ...
    jq_grid.jqGrid({
        ...
        hidegrid:false,
        footerrow : true,
        pager: '#grid_detalle_importacion_pager',
        ...
        onCellSelect : function(rowid, iCol, cellcontent, e) { // use this event to capture edited cellID
            ...
            var cm = jq_grid.jqGrid("getGridParam", "colModel");
            if(cm[iCol].name !="Signo" && cm[iCol].name !="Cencos" && cm[iCol].name !="Ot"){
                jq_grid.resetSelection();
                row_editing_detalle_importacion = iRow;
                selected_cell_detalle_ventas = iCol;
                jq_grid.jqGrid('setGridParam',{
                    cellEdit: true,
                });
            }else{
                jq_grid.jqGrid('saveCell',row_editing_detalle_importacion,selected_cell_detalle_ventas);
                jq_grid.jqGrid('setGridParam',{
                    cellEdit: false,
                });
            }
...
async function agregar_detalle_ventas(){
    console.log('asgdjhasgdasdasd');
    var jq_grid=jq_grid_detalle_importacion;
    var id_selected = jq_grid.jqGrid('getGridParam', 'selrow');
    for(var i=0; i<lista_productos_elegidos.length;i++){
        ...
        await $.ajax({
            ...
            data: {
                ...
                let element = data[0];
                if (element.Afecta_Advalorem == 'S') {
                    porcentaje_advalorem = element.Porcenteje_Advalorem;
                }else{
                    porcentaje_advalorem = 0;
                }
                
            } 
        });
        console.log(porcentaje_advalorem);        
        ...
        new_fila.Adv_Porcentaje = porcentaje_advalorem;
...
function recalcular_montos_importacion(Campo){
    console.log('jghasjdasdasdasd');
    var jq_grid=jq_grid_detalle_importacion;

    var dataIDs = jq_grid.getDataIDs(); 

    var Incoterm = 0, Total = 0, Flete = 0, Seguro = 0,IPM = 0, IGV = 0, Advalorem = 0;
    ...
    for(i = 0; i < dataIDs.length; i++)
    {
        ...
        if(rowdata.Nombre!=""){
            ...
            var data = {
                ...
                Tc: Number(jq_tasa_cambio.val()),
                Cantidad: Number(rowdata.Cantidad),
                Precio: Number(rowdata.Unit),
                Flete: Number(jq_flete.val()),
                Incoterm: Number(jq_numero_inconterm.val()),
                Seguro: Number(rowdata.Seguro),
                Otros: Number(rowdata.Otros),
                Porcentaje_Advalorem: rowdata.Adv_Porcentaje,
                Convenio: rowdata.Conv_Me,
                Signo: rowdata.Signo
            };

            var calculos = calcular_totales_importacion(data);
            console.log(calculos);
            
            jq_grid.jqGrid('setRowData', rowid, {
                Importe : (calculos.Incoterm + calculos.Flete + calculos.Seguro),
                Flete_Unit : calculos.Flete_Unit,//((calculos.Flete / Incoterm) * calculos.Incoterm) / rowdata.Cantidad,
                Flete: calculos.Flete,
                // Adv_Mn_Unit : calculos.Advalorem_Mn,
                // Adv_Me_Unit : calculos.Advalorem_Me,
                // Total_Adv_Mn : calculos.Total_Advalorem_Mn,
                // Total_Adv_Me : calculos.Total_Advalorem_Me
            });

            // if (jq_moneda.val() == "S/") {
            //     Advalorem = Advalorem + calculos.Total_Advalorem_Mn
            // }else{
            //     Advalorem = Advalorem + calculos.Total_Advalorem_Me
            // }
            Incoterm = Incoterm* 1 + calculos.Incoterm;
            Total = Incoterm + Number(jq_flete.val()) + Number(rowdata.Seguro);
            //Flete = Flete*1 + calculos.Flete;
            Seguro = Seguro*1 + calculos.Seguro; 
            // IPM =  Total * 0.02;
            // IGV = Total * 0.16;
            // Percepcion_Total = (Total + IPM + IGV) 
        ...
        rowdata = jq_grid.getRowData(rowid);
        ...
        jq_grid.jqGrid('setCell',rowid,'Flete','',{color: rowdata.Flete==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Seguro','',{color: rowdata.Seguro==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Otros','',{color: rowdata.Otros==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Adv_Porcentaje','',{color: rowdata.Adv_Porcentaje==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Sobretasa','',{color: rowdata.Sobretasa==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Conv_Mn','',{color: rowdata.Conv_Mn==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Conv_Me','',{color: rowdata.Conv_Me==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Fob_Otros','',{color: rowdata.Fob_Otros==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Adv_Mn_Unit','',{color: rowdata.Adv_Mn_Unit==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Adv_Me_Unit','',{color: rowdata.Adv_Me_Unit==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Total_Adv_Mn','',{color: rowdata.Adv_Mn_Unit==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Total_Adv_Me','',{color: rowdata.Adv_Me_Unit==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Porc_Isc','',{color: rowdata.Porc_Isc==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Monto_Isc','',{color: rowdata.Monto_Isc==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Sobretasa_Mn_Unit','',{color: rowdata.Sobretasa_Mn_Unit==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Sobretasa_Me_Unit','',{color: rowdata.Sobretasa_Me_Unit==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Flete_Unit','',{color: rowdata.Flete_Unit==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Seguro_Unit','',{color: rowdata.Seguro_Unit==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Otros_Unit','',{color: rowdata.Otros_Unit==0 ? 'white' : 'black'});
        jq_grid.jqGrid('setCell',rowid,'Cantidad_Kardex','',{color: rowdata.Cantidad_Kardex==0 ? 'white' : 'black'});
    }

    
    $("#advalorem").val(Advalorem.toFixed(2));
    $("#ipm").val(IPM.toFixed(2));
    $("#igv").val(IGV.toFixed(2));

    $("#numero_inconterm").val(Incoterm.toFixed(2));
    if(Campo !='Flete') $("#flete").val(Flete.toFixed(2));
    $("#seguro").val(Seguro.toFixed(2));
    $("#total_cif").val(Total.toFixed(2));
    ...
    $.ajax({
        ...
        success: function (result){
            ...
            if(result.length>0){
                ...
                copiado==true ? recalcular_montos_importacion() : '';
...
function lista_select_signo(cellvalue, options, rowObject){
    var newlista_ot = lista_signo.replace('value ="'+cellvalue+'"','value ="'+cellvalue+'" selected');
    return '<select id="lista_select_ot_'+options.rowId+'" class="form-control form-control-sm c_mantenimientos select-sm" tabla="orden_trabajo">'+newlista_ot+'</select>'
}